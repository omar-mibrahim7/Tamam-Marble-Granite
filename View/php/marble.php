...
$perPage = 16;
$totalCount = Product::countByType($conn, 'Marble');
$totalPages = max(1, (int)ceil($totalCount / $perPage));
$currentPage = isset($_GET['page']) ? (int)$_GET['page'] : 1;
$currentPage = max(1, min($currentPage, $totalPages));
$offset = ($currentPage - 1) * $perPage;
$products = Product::findByType($conn, 'Marble', $perPage, $offset);
?>
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <title>Marble</title>
  <link rel="stylesheet" href="../css/marble.css?v=<?php echo filemtime('../css/marble.css'); ?>">
...
<!-- Results Count -->
<p class="results">Showing all <?php echo $totalCount; ?> results &gt;&gt;</p>

<!-- Products Grid -->
<div class="grid">
  <?php if (empty($products)): ?>
    <p style="color:#888;">No marble products found.</p>
  <?php endif; ?>
  ...
  <?php foreach ($products as $product): ?>
    ...
    <div class="card">
      <img class="card-img"
           src="<?php echo h(productListImageUrl($product['image'], '../pic/black&gray marble.jpeg')); ?>"
           alt="<?php echo h($productName); ?>">
      <h3><?php echo h($productName); ?></h3>
      <div class="rating">
        5.0 <i class="fa-solid fa-star"></i>
      </div>
      <a href="mardetails.php?id=<?php echo $productId; ?>" class="btn">View More</a>
    </div>
  <?php endforeach; ?>
</div>
...
<div class="pagination">
  <?php if ($currentPage > 1): ?>
    <a href="marble.php?page=<?php echo $currentPage - 1; ?>"><button class="circle-btn">&lt;</button></a>
  <?php else: ?>
...
<script src="../js/marble.js?v=<?php echo filemtime('../js/marble.js'); ?>"></script>
