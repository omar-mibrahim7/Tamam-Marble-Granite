...
$product = null;
$productImg = "../pic/mix marble2.jpeg";
$productInDb = false;

$dbProduct = Product::findById($conn, $id);

if ($dbProduct && $dbProduct['product_type'] === 'Granite') {
    $productInDb = true;
    $product = [
    ...
    $productImg = detailImageUrl($dbProduct['image'], "../pic/mix marble2.jpeg");
} else {
    header("Location: granite.php");
    exit;
}
...
if ($description === '') {
    $description = $product['name'] . ' features a natural granite texture with unique patterns. Ideal for flooring, wall cladding, kitchen countertops, and interior spaces.';
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <title><?php echo h($product['name']); ?> - Granite Details</title>
  <link rel="stylesheet" href="../css/gradetails.css?v=<?php echo filemtime('../css/gradetails.css'); ?>">
...
<div class="details-container">
  ...
  <div class="right">
    ...
    <div class="info-box">
      <p class="category">GRANITE | <?php echo h($product['name']); ?></p>
...
<div class="table">
  ...
  <table>
    ...
    <tr><td>Scratch Resistance</td><td><?php echo h($product['scratch_resistance'] ?: 'High'); ?></td></tr>
...
<script src="../js/gradetails.js?v=<?php echo filemtime('../js/gradetails.js'); ?>"></script>
</body>
</html>
